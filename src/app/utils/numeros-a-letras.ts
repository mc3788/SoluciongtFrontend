...
// Unidades()
  ...
  private DecenasY(strSin, numUnidades): string {
    if (numUnidades > 0) {
  ...
  private Seccion(num, divisor, strSingular, strPlural): string {
  ...
  numerosALetras(num, currency): string {
    currency = currency || {};
    const data = {
      numero: num,
    ...
    // }
    data.letrasCentavos =  (data.centavos < 10 ? '0' : '') + data.centavos + '/100';
    // }
